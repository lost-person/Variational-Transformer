...
def read_langs(vocab: Lang):
    ...
    with open('./data/cornell/vocab.ori', 'r', encoding='utf-8') as f:
        for word in f:
            vocab.index_word(word.strip())

    train_context = load_list_from_path('./data/cornell/train/context.txt')
    train_target = load_list_from_path('./data/cornell/train/response.txt')

    dev_context = load_list_from_path('./data/cornell/dev/context.txt')
    dev_target = load_list_from_path('./data/cornell/dev/response.txt')
    
    test_context = load_list_from_path('./data/cornell/test/context.txt')
    test_target = load_list_from_path('./data/cornell/test/response.txt')
...
def load_dataset():
    if(os.path.exists('./data/cornell/dataset_cornell.p')):
        print("LOADING cornell")
        with open('./data/cornell/dataset_cornell.p', "rb") as f:
        ...
        with open('./data/cornell/dataset_cornell.p', "wb") as f:
