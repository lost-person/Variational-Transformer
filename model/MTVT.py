...
class Latent(nn.Module):
    def __init__(self, hidden_size, num_heads, total_key_depth, total_value_depth,
        ...
        self.query = nn.Parameter(torch.randn(config.batch_size, config.max_seq_len, config.hidden_dim))
        self.dec = DecoderLayer(*params)
        ...
        self.var = PositionwiseFeedForward(config.hidden_dim, config.filter, config.hidden_dim,
                                                                 layer_config='lll', padding = 'left', 
                                                                 dropout=0)
        self.mean_p = PositionwiseFeedForward(config.hidden_dim, config.filter, config.hidden_dim,
                                                                 layer_config='lll', padding = 'left', 
                                                                 dropout=0)
        self.var_p = PositionwiseFeedForward(config.hidden_dim, config.filter, config.hidden_dim,
                                                                 layer_config='lll', padding = 'left', 
                                                                 dropout=0)
    ...
    def forward(self, x, x_p, mask, mask_p, train=True):
        ...
        if x_p is not None:
            posterior, _, attn_dist_p, _ = self.var_dec((self.query, x_p, [], (mask_p, None)))
...
class CvaeTrans(nn.Module):
    ...
    def __init__(self, vocab, emo_number, model_file_path=None, is_eval=False, load_optim=False):
        ...
        self.decoder = Decoder(config.emb_dim, hidden_size=config.hidden_dim,  num_layers=config.hop, 
                                num_heads=config.heads, total_key_depth=config.depth, 
                                total_value_depth=config.depth, filter_size=config.filter)
        
        self.latent_layer = Latent(config.hidden_dim, num_heads=config.heads, total_key_depth=config.depth, 
                                total_value_depth=config.depth, filter_size=config.filter, vocab_size=self.vocab_size)

        self.generator = Generator(config.hidden_dim, self.vocab_size)
        self.bow = SoftmaxOutputLayer(config.hidden_dim, self.vocab_size)

        self.linear1 = nn.Linear(2 * config.hidden_dim, config.hidden_dim)
        self.linear2 = nn.Sequential(nn.Linear(2 * config.hidden_dim, config.hidden_dim), nn.Tanh())
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        if word_encoder_hidden.size(-1) != config.hidden_dim:
            word_encoder_hidden = self.linear1(word_encoder_hidden)
        ...
        if config.model=="cvaetrs":
            kld_loss, z = self.latent_layer(encoder_outputs, pre_logit, mask_src, mask_trg, train=True)
        
        z = z[:, :pre_logit.size(1), :]
        pre_logit = self.linear2(torch.cat((z, pre_logit), dim=-1))
        ...
        if config.model=="cvaetrs":
            z_logit = self.bow(z) # [batch_size, seq_len, vocab_size]
    ...
    def decoder_greedy(self, batch, max_dec_step=50):
        ...
        if config.model=="cvaetrs":
            kld_loss, z = self.latent_layer(encoder_outputs, None, mask_src, None, train=False)
